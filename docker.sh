...
docker run -d --name "opcon-core" --hostname "opcon-core" --net="opcon-network" --restart=always -e "DatabaseName=${DatabaseName:=OpConxps}" -e "OpConxpsDBServerName=opcon-mssql" -e "OpConxpsSQLInstance=opcon-mssql" -e "DBLogicalDataFilename=${DatabaseName:=OpConxps}"_Data -e "DBLogicalLogFilename=${DatabaseName:=OpConxps}_Log" -e "PathToDatabaseDataFile=/var/opt/mssql/data/${DatabaseName:=OpConxps}_Data.MDF" -e "PathToDatabaseLogFile=/var/opt/mssql/data/${DatabaseName:=OpConxps}_Log.LDF" -v ${DOCKER_PATH}/opcon/config:/app/config -v ${DOCKER_PATH}/opcon/log:/app/log -p 9010:9010 -p 8181:8181 smaengineering.azurecr.io/opcon:$DOCKER_OPCON_IMAGE_VERSION -c -d -u "sa" -p "${DatabasePassword}" -P "${DatabasePasswordEncrypted}" -l "${LicenseKey}" -L "${LicenseName:=0}"
